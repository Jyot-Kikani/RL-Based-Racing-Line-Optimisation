...
def _steps_from_checkpoint(path: str) -> int:
    """Parse the step count embedded in a checkpoint filename, e.g.
    stage2_sac_190000_steps.zip → 190000.  Returns 0 if not parseable."""
    import re
    m = re.search(r"_(\d+)_steps", os.path.basename(path))
    return int(m.group(1)) if m else 0


def train(stage: int, steps_override: int = None, n_envs: int = 1,
          headless: bool = False, resume_from: str = None):
    import config as cfg_mod
    cfg = STAGES[stage]
    # Work out how many steps to train
    if steps_override is not None:
        steps = steps_override
    elif resume_from is not None:
        already_done = _steps_from_checkpoint(resume_from)
        steps = max(0, cfg["steps"] - already_done)
        print(f"  Resuming from {os.path.basename(resume_from)} "
              f"({already_done:,} steps done → {steps:,} remaining)")
    else:
        steps = cfg["steps"]
    ...
    ckpt_cb = CheckpointCallback(
        save_freq            = max(1_000, 5_000 // n_envs),   # checkpoint every ~5k steps
    ...
    # ── Train ─────────────────────────────────────────────────────────────────
    # When resuming we never reset the step counter so TensorBoard curves
    # continue from where they left off.
    reset_steps = (stage == 1) and (resume_from is None)
    model.learn(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume-from", type=str,  default=None,
                        help="Path to a checkpoint .zip to resume from. "
                             "Remaining steps are inferred from the filename "
                             "(e.g. stage2_sac_190000_steps.zip → 210k left).")
    args = parser.parse_args()
    train(args.stage, args.steps, args.n_envs, args.headless, args.resume_from)